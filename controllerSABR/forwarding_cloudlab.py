...
class ofp_match_withHash(of.ofp_match):
    ...
    def __hash__(self):
        return hash((self.wildcards, self.in_port, self.dl_src, self.dl_dst, self.dl_vlan, self.dl_vlan_pcp,
                     self.dl_type, self.nw_tos, self.nw_proto, self.nw_src, self.nw_dst, self.tp_src, self.tp_dst))
    ...
    for i in range(len(s_keys)):
        for j in range(len(s_keys[i])):
            ...
            for k in range(0, len(s_lsps[s_keys[i][j]][2]), 3):
                sum_bw = 0.0
                # arima_avg=0.0
                for res in table.find(
                        {"dpid": str(s_lsps[s_keys[i][j]][2][k]), "portno": s_lsps[s_keys[i][j]][2][k + 2]}).sort(
        ...
        post = {"server_ip": server_list[j], "client_ip": clientip_list[i], "min_bw": (min_bw), "hop_count": hop_count,
...
def _install_shortestpath(match):
    ...
    for i in range(0, len(cross_path) - 2, 3):
        ...
        log.debug("CROSS_TRAFFIC: Installing forward from switch %s to output port %s", cross_path[i],
                  cross_path[i + 1])
        ...
        log.debug("CROSS_TRAFFIC: Installing reverse from switch %s to output port %s", cross_path[i],
                  cross_path[i + 2])
...
def _install_arp(conn_dpid, match, init_event, src, ofp_in_port):
    ...
    for i in range(0, len(s_lsps[best_path][2]) - 2, 3):  # for switch in path.keys():
        ...
        if str(match.nw_src) in client_port.keys():
            ...
            log.debug("Installing Forward from switch %s to switch %s: output port %s", src_sw, dst_sw,
                      s_lsps[best_path][2][i + 1])
            ...
            msg.idle_timeout = 10
            msg.flags = of.OFPFF_SEND_FLOW_REM
            msg.match = of.ofp_match(in_port=s_lsps[best_path][2][i + 1], dl_type=0x0806, nw_src=match.nw_src,
                                     nw_dst=match.nw_dst)

            msg.out_port = cl_port
            log.debug("Installing Reverse1 from switch %s to switch %s : input port %s and output port %s", src_sw,
                      dst_sw, msg.match.in_port, msg.out_port)
...
def _install_path(conn_dpid, match, init_event, src, ofp_in_port):
    ...
    for i in range(0, len(s_lsps[best_path][2]) - 2, 3):  # for switch in path.keys():
        ...
        if str(match.nw_src) in client_port.keys():
            ...
            msg.idle_timeout = 10
            msg.flags = of.OFPFF_SEND_FLOW_REM
            msg.match = of.ofp_match(in_port=s_lsps[best_path][2][i + 1], dl_type=0x0800, nw_src=match.nw_src,
                                     nw_dst=match.nw_dst)

            msg.out_port = cl_port
            log.debug("Installing Reverse from switch %s to switch %s : input port %s and output port %s", src_sw,
                      dst_sw, msg.match.in_port, msg.out_port)
            msg.actions.append(of.ofp_action_output(port=cl_port))
            poxcore.openflow.sendToDPID(poxutil.str_to_dpid(cur_sw), msg)
            '''
...
class Switch(EventMixin):
    ...
    def _handle_PacketIn(self, event):
        ...
        if (event.dpid,
            event.port) not in switch_ports:  # only relearn locations if they arrived from non-interswitch links
        ...
        if packet.effective_ethertype == packet.LLDP_TYPE:
            ...
            if packet.type == packet.IP_TYPE:
                log.debug("Switch %s received PacketIn of type 0x%0.4X, received from %s.%s", self,
